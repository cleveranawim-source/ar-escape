...
function showStart() {
  ...
  // 기본은 잠금이다. 연습 모드는 마커 사진을 전부 펼쳐 보여주기 때문에
  // 야외 활동에서는 "어디로 가야 하는지" 를 통째로 알려주는 치트가 된다.
  // 방 코드가 없는 주소를 그대로 공유해도 안전하도록, 켜는 쪽을 명시하게 한다.
  // (AR 이 아예 컴파일되지 않았다면 연습 모드 말고는 할 방법이 없으므로 예외)
  const practiceOk = !sc.mind || S.forcePractice;
